...
def test_lexer_string():
    ...
    assert str_token.value == "Hello, ' World!", f"Got unexpected string - {str_token.value}"
...
def test_lexer_numeric():
    ...
    assert lexer.next_token().type == "NewLine", f"Got unexpected token instead of newline!"
